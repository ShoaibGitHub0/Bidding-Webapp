...
from .models import AuctionItem, Category,Bid, Watchlist,Comment, Winner,DeliveryDetails
from .serializers import AuctionItemSerializer, CategorySerializer, BidSerializer, WatchlistSerializer, WatchlistDetailSerializer,CommentSerializer,WinnerAuctionSerializer, DeliveryDetailSerializer,AuctionItemCreateSerializer,WinnerAuctionGetSerializer,DeliveryDetailGetSerializer
...
class PaymentListCreateView(generics.ListCreateAPIView):
    ...
    def get(self, request):
        paid_auction = DeliveryDetails.objects.filter(paid=True,user__winner=self.request.user)
    ...
    def post(self, request, winner_id):
        ...
        if winner.winner == user:
            if serializer.is_valid():
                serializer.save(user=winner,paid=True)
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            else:
            # If the serializer is invalid, return the error messages
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        else:
            raise ValidationError("Winner Can Only Pay Amount")
